...
function App() {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      setError(`Erro ao executar tradução: ${error.message} `);
    } finally {
      setIsLoading(false);
    }
  }
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="bg-white shadow-sm">
        <div className="max-w-5xl mx-auto px-4 py-3 flex items-center ">
          <h1 className="text-headerColor text-2xl font-bold">TraduzAí</h1>
        </div>
      </header>
      ...
      <main className="flex-grow flex items-start justify-center px-4 py-8 ">
        ...
        <div className="w-full max-w-5xl bg-white rounded-lg shadow-md overflow-hidden">
          <div className="flex items-center justify-between p-4 border-b border-gray-200">
            ...
            <button className="p-2 rounded-full hover:bg-gray-100 outline-none" onClick={swipeTranslate}>
              <svg
          ...
          {error && (<div className='p-4 bg-red-100 border-top border-red-400 text-red-700'>
            {error}
          </div>)
      ...
      <footer className="bg-white border-t border-gray-200 mx-auto w-full flex justify-center items-center">
        <div className="mx-w-5xl mx-auto px-4 py-3 text-sm text-headerColor">
          &copy; {new Date().getFullYear()} TraduzAí
        </div>
      </footer>
    </div>
  )
